...
C --> G[Conversion to FP8 Checkpoint]
G --> F[Direct serving]
D --> F
```
...
The following sections cover how the checkpoints are quantized to FP8.

### Conversion to FP8 checkpoint

The most straightforward way to convert a higher precision model to FP8 is to use a library like [llm-compressor](https://github.com/vllm-project/llm-compressor), which offers an option to convert BF16 checkpoints to FP8. The process is mostly straightforward, where the scales of the weight tensors are calculated and stored in the state dict. These scales can be per-tensor or blockwise, similar to what we saw in training.

However, FP8 GEMM still requires activations to be converted to FP8. Activations are quantized to FP8 on the fly. This usually degrades the performance gains one can expect from FP8 inference due to scale calculation of activations before performing GEMM.
