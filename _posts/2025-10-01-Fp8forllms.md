...
category: [llm-optimization, gpus]
date: 2025-10-01
---
...
4. There is no "standard" recipe to do FP8 training.
...
Ant group goes a step further in [their Zhihu post](https://zhuanlan.zhihu.com/p/1956374947229304351) and designs their own fused kernels for FP8 reporting >50% gains in training throughput.
...
1. Quantizing weights: Quantization of weights is similar to dynamic PTQ, where weight tensors are converted to FP8, and the calculated scales are stored in the state dict along with the corresponding FP8 weights.
2. Calibration: Run a small, representative dataset through the original, high-precision model. During this step, the dynamic range (i.e., the minimum and maximum absolute values) of the activations is recorded.
3. Scale Calculation: Compute a fixed, static scaling factor for each tensor (per-tensor or blockwise) based on those ranges. These scales (called as activation scales) are stored in the state dict along with the model weights.
...
1. [Lecture 30: Quantized Training](https://www.youtube.com/watch?v=Br07GsnnvWc)
2. [Unified FP8: Moving Beyond Mixed Precision for Stable and Accelerated MoE RL](https://lmsys.org/blog/2025-11-19-fp8-rl/)
